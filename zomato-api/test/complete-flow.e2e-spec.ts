...
describe('Complete Zomato Flow (E2E)', () => {
  ...
  const registerUser = async (user: any) => {
    ...
    if (res.status === 201 || res.status === 200) {
      ...
      console.log(
        `Registration failed for ${user.email}: ${res.status}`,
        res.body,
      );
      ...
      if (login.status === 200 || login.status === 201) {
        ...
        console.log(
          `Login failed for ${user.email}: ${login.status}`,
          login.body,
        );
      }
    }
  ...
  it('5. Place Order (Customer) -> Verify WebSocket', async () => {
    ...
    const notificationPromise = new Promise<void>((resolve) => {
      restaurantSocket.on('order:new', (_data) => resolve());
    ...
    expect(res.status).toBe(201);
    orderId = res.body.id;
    _orderNumber = res.body.orderNumber;
  ...
  it('6. Accept Order (Restaurant) -> Verify Customer WS', async () => {
    const customerPromise = new Promise<void>((resolve) => {
      socket.on('order:accepted', (_data) => resolve());
