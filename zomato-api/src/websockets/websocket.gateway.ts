  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      console.log(
        `✅ User ${payload.sub} (${payload.role}) connected - Socket: ${client.id}`,
      );
    } catch (error) {
  ...
  emitToAvailableDeliveryPartners(
  ...
  notifyOrderReady(order: any) {
    ...
    this.emitToAvailableDeliveryPartners(
  ...
  handleMessage(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    data: { orderId: string; message: string; recipientId: string },
  ) {
