...
export class OrderJobsProcessor {
  ...
  async handleAutoCancel(_job: Job) {
  ...
  async monitorStuckOrders(_job: Job) {
    ...
    if (stuckOrders.length > 0) {
      this.logger.warn(
        `Found ${stuckOrders.length} stuck orders! IDs: ${stuckOrders.map((o) => o.id).join(', ')}`,
      );
  ...
  handleAssignment(job: Job<{ orderId: string }>) {
