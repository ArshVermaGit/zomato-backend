...
import {
  OnboardDeliveryPartnerDto,
  UpdateVehicleDto,
} from './dto/delivery-onboarding.dto';
import { S3Service } from '../../common/services/s3.service';
import { LocationService } from '../location/location.service';
import { OrderStateService } from '../orders/order-state.service';
import { EarningsService } from './earnings.service';
import { OnboardingStatus, DocumentType, DocumentStatus } from '@prisma/client';
...
export class DeliveryService {
  ...
  async updateDocumentStatus(userId: string, docTypeKey: string, url: string) {
    ...
    if (!partner)
      throw new NotFoundException(
        'Partner record not found. Please onboard first.',
      );
  ...
  async getStatus(userId: string) {
    ...
    partner.documents.forEach((doc) => {
      ...
      const key = Object.keys(this.docTypeMap).find(
        (k) => this.docTypeMap[k] === doc.type,
      );
