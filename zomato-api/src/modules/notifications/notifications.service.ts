...
export class NotificationsService {
    ...
    async sendPushNotification(userId: string, notification: {
        ...
        try {
            const response = await admin.messaging().send(message as any); // sendMulticast in newer node sdk, or use tokens field with sendMulticast
            // Actually per docs: sendMulticast({ tokens: [], ... })
            const multicastResponse = await admin.messaging().sendEachForMulticast(message as any);
