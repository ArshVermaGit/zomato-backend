...
export class PromoValidationService {
    ...
    async validatePromo(promo: Promo, userId: string, cartValue: number, restaurantId: string): Promise<{ valid: boolean; reason?: string }> {
        const now = new Date();

        // 1. Basic Checks
        if (!promo.isActive) return { valid: false, reason: 'Promo code is inactive' };
        if (now < promo.validFrom || now > promo.validUntil) return { valid: false, reason: 'Promo code has expired' };
        if (promo.usageLimit && promo.usedCount >= promo.usageLimit) return { valid: false, reason: 'Promo usage limit reached' };
        if (cartValue < Number(promo.minOrderValue)) return { valid: false, reason: `Minimum order value of ₹${promo.minOrderValue} required` };
        ...
        if (promo.isNewUserOnly) {
            ...
            if (orderCount > 0) return { valid: false, reason: 'Promo code valid for new users only' };
        }
