...
export class RefundsService {
    ...
    async processRefund(orderId: string, type: 'GATEWAY' | 'WALLET' = 'GATEWAY', reason?: string) {
        ...
        if (refundCalculation.refundAmount <= 0) {
            throw new BadRequestException('Refund amount is zero based on order status');
        }
    ...
    private async processGatewayRefund(paymentId: string, razorpayPaymentId: string, amount: number, reason?: string) {
        // Call Razorpay
    ...
    private async processWalletRefund(userId: string, paymentId: string, amount: number, reason?: string) {
        ...
        throw new BadRequestException('Customer Wallet not currently supported. Use GATEWAY.');
    }
}
