import { Controller, Post, Put, Get, Body, Param, Query, UseGuards, Request, ParseUUIDPipe } from '@nestjs/common';
...
export class OrdersController {
    ...
    async rateOrder(@Request() req, @Param('id', ParseUUIDPipe) id: string, @Body() dto: CreateRatingDto) {
    ...
    async prepareOrder(@Request() req, @Param('id', ParseUUIDPipe) id: string) {
        return this.orderStateService.transition(id, OrderStatus.PREPARING, req.user.userId, req.user.role);
    }
    ...
    async assignOrder(@Param('id', ParseUUIDPipe) id: string, @Body() dto: AssignDeliveryPartnerDto) {
    ...
    async pickupOrder(@Request() req, @Param('id', ParseUUIDPipe) id: string) {
        return this.orderStateService.transition(id, OrderStatus.PICKED_UP, req.user.userId, req.user.role); // TODO: Pass deliveryPartner ID handled in service
    }
    ...
    async cancelOrder(@Request() req, @Param('id', ParseUUIDPipe) id: string, @Body() dto: CancelOrderDto) {
        return this.orderStateService.transition(id, OrderStatus.CANCELLED, req.user.userId, req.user.role, dto.reason);
    }
}
