...
export class OrdersService {
  ...
  async createOrder(customerId: string, dto: CreateOrderDto) {
    ...
    for (const item of dto.items) {
      ...
      orderItems.push({
        ...
        selectedModifiers:
          itemWithModifiers.selectedModifiers || Prisma.JsonNull,
    ...
    if (dto.promoCode) {
      ...
      if (!validation.valid) {
        ...
        throw new BadRequestException(
          `Invalid promo code: ${validation.reason}`,
        );
      }
    ...
    const totalAmount =
      itemsTotal + deliveryFee + taxes - discount + (dto.tip || 0);
    ...
    if (dto.paymentMethod !== 'COD') {
      // Using string literal matching DTO enum map or raw
  ...
  async updateDeliveryLocation(
    ...
    if (order) {
      ...
      this.realtimeGateway.notifyLocationUpdate(
        orderId,
        order.customerId,
        location,
      );
